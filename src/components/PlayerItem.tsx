...
export const FindImg = (proops: { name: string }) => {
  let imgPath = ''
  try {
...
const PlayerItem = (props: { player: playerType; onPlayerChange: any; squad: boolean }) => {
  return (
    <>
      {props.player.sklad === props.squad ? (
        <li className='bg-gray-50 m-5 p-2 flex flex-row basis-1 w-80 justify-between shadow-xl rounded-lg'>
          <div className='flex flex-row items-center '>
            ...
            <input className='w-10 text-xl' placeholder={props.player.numerZawodnika.toString()} />
            <p className='flex m-1 text-lg'>{props.player.nazwa}</p>
          </div>
